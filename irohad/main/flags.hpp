...
inline void addPeerFlags(CLI::App *p,
  ...
  p->add_option("--pubkey"s, crypto.public_key, "Peer's public key"s, false)
      ->required()
      ->envname("IROHA_PUBKEY"s)
      ->group("Peer"s)
      ->check(CLI::ExistingFile);

  p->add_option("--privkey"s, crypto.private_key, "Peer's private key"s, false)
      ->required()
...
inline void addPostgresFlags(CLI::App *p, iroha::config::Postgres &postgres) {
  ...
  p->add_option("--pgdatabase"s, postgres.database, "PostgreSQL database name"s, true)
      ->required()
