...
namespace flag {
  ...
  enum { IROHA_SIZE_MIN = 1, IROHA_SIZE_MAX = 100000 };
  ...
  inline void addPeerFlags(CLI::App *p,
    ...
    p->add_option("--privkey"s,
                  crypto->private_key,
                  "Path to peer's private key"s,
                  false)
        ->required()
  ...
  inline void addPostgresFlags(CLI::App *p,
    ...
    p->add_option(
         "--pgport"s, postgres->port, "PostgreSQL database port."s, true)
    ...
    p->add_option(
         "--pguser"s, postgres->username, "PostgreSQL username"s, false)
        ->required()
